...
public class BG : MonoBehaviour {
    ...
    void Start () {
        UFO.transform.position = new Vector2(UFO.transform.position.x, -10f);
        Shuttle.transform.position = new Vector2(Shuttle.transform.position.x, -10f);
    }
        ...
        if (player.transform.position.y == -2f && player)
        {
            ...
            if (!bUFO || !bShuttle)
            {
                ...
                if (ver == 43 && !bUFO)
                {
                    ...
                    desiredPosUFO = new Vector2(UnityEngine.Random.Range(-1.6f, 1.6f), -10f);
                }
                if (ver == 1200 && !bShuttle)
                {
                    bShuttle = true;
                    Shuttle.transform.position = new Vector2(UnityEngine.Random.Range(-1.6f, 1.6f), 10f);
                    desiredPosShuttle = new Vector2(UnityEngine.Random.Range(-1.6f, 1.6f), -10f);
                }
            }
            if (UFO.transform.position.y > -6f) UFO.transform.position = Vector2.MoveTowards(UFO.transform.position, desiredPosUFO, speed * Time.deltaTime);
            else bUFO = false;
            if (Shuttle.transform.position.y > -6f) Shuttle.transform.position = Vector2.MoveTowards(Shuttle.transform.position, desiredPosShuttle, speed * Time.deltaTime);
            else bShuttle = false;
